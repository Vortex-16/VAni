...
export async function sendVerificationEmail(to: string, code: string, name: string): Promise<boolean> {
  const subject = "Verify your email - Discharge Buddy";
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; borderRadius: 8px;">
      <h2 style="color: #7C3AED; text-align: center;">Welcome to Discharge Buddy!</h2>
      <p>Hello ${name},</p>
      <p>Thank you for registering with Discharge Buddy. To complete your sign-up and secure your account, please verify your email address using the 6-digit code below:</p>
